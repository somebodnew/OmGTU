...
print("Коэффициенты 1/П(x_i - x_j):")
for i in range(n):
    den = 1.0
    for j in range(n):
        if j != i:
...
print("\nЛинейный сплайн ph(x) = a_k*x + b_k на каждом отрезке:\n")
print(f"  {'Отрезок':>10}  {'a_k':>10}  {'b_k':>10}")
separator("-", 50)
...
print(f"  {'xi':>8}  {'yi':>10}  {'ph(xi)':>10}  {'погрешн.':>12}")
for i in range(n):
...
print("\nКвадратичный сплайн ph(x) = a_k*x^2 + b_k*x + c_k:\n")
print(f"  {'Отрезок':>10}  {'a_k':>10}  {'b_k':>10}  {'c_k':>10}")
separator("-", 60)
